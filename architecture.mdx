...
<CardGroup cols={2}>
  ...
  <Card title="LogicScript DSL" icon="terminal">
    **Package:** `tautoteacher2.logicscript`
    ...
    - **`LogicScriptRegressionHarness`** — self-contained test runner with 39 cases; exits non-zero on any failure.
...
- **`ServicioTautoTeacher`** (in `tautoteacher2.servicio`) is the placeholder for a higher-level facade that can orchestrate both formula and NL analysis through typed DTOs (`SolicitudAnalisis`, `ResultadoAnalisis`), intended for future extension without changing existing callers.
